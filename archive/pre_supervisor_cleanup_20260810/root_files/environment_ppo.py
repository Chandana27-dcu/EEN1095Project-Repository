...
class NetworkSlicingPPOEnv(gym.Env):
    ...
    def _calculate_reward(
        ...
        """
        Calculate the PPO reward using only the three required QoS metrics:

            40% throughput satisfaction
            35% latency satisfaction
            25% packet-loss-ratio satisfaction

        The continuous allocation action is intentionally not rewarded or
        penalized directly. There are no fairness, queue, starvation, SLA,
        packet-drop, or resource-utilization terms in this reward.
        """
        del action  # Allocation affects network performance, not reward directly.
        ...
        for slice_name in self.slices:
            ...
            throughput_score = float(
                np.clip(throughput_percent / 100.0, 0.0, 1.0)
            )
            ...
            latency_score = float(
                np.clip(
                    1.0
                    - average_latency_ms / max(required_latency_ms, 1e-8),
                    0.0,
                    1.0,
                )
            )
            ...
            plr_score = float(
                np.clip(
                    1.0 - plr_percent / max(required_plr, 1e-8),
                    0.0,
                    1.0,
                )
            )
            ...
            slice_rewards.append(float(slice_reward))
