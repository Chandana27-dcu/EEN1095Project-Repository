...
from src.actions_common import (
    ACTIONS,
    get_rb_allocation,
)
from src.config_ppo import CONFIG
from src.traffic_common import (
    generate_traffic,
)
...
class NetworkSlicingPPOEnv(gym.Env):
    """
    PPO environment for dynamic network-slice resource allocation.
    ...
    Action space:
        ...
        PPO selects one action index from the same common
        action set used by D3QN.
    ...
    def __init__(self) -> None:
        ...
        if int(CONFIG["STATE_SIZE"]) != (
            ...
            raise ValueError(
                ...
                "number of slices x 6 state features."
            )
    ...
    def step(
        ...
        if not self.action_space.contains(
            ...
            raise ValueError(
                f"Invalid PPO action index: "
                f"{action_index}"
            )

        # -----------------------------------------------------
        # Selected common allocation
        # -----------------------------------------------------
        ...
        # -----------------------------------------------------
        # Convert allocation percentage into RBs
        # -----------------------------------------------------
